...
parser.add_argument('--device', type=str, default='0', help="CUDA_VISIBLE_DEVICES, i.e. which GPU index to use. Pass 'cpu' to force the whole pipeline (ESM embedding, inference, relaxation) onto CPU only, even if a GPU is present.")
...
args.no_gpu = args.device.strip().lower() == 'cpu'
...
# --no_gpu forces every downstream subprocess onto CPU: CUDA_VISIBLE_DEVICES is
# cleared so no child process can see a GPU even if one exists, and each script
# also gets an explicit --no_gpu/--nogpu flag so it doesn't just fall back to
# CPU on failure but is asked to run on CPU deliberately (this matters for
# relax_final.py, which otherwise hard-requests the CUDA OpenMM platform and
# silently drops its output on machines without one).
if args.no_gpu:
    cuda_env_prefix = "CUDA_VISIBLE_DEVICES="
    no_gpu_flag = " --no_gpu"
    esm_no_gpu_flag = " --nogpu"
else:
    cuda_env_prefix = f"CUDA_VISIBLE_DEVICES={args.device}"
    no_gpu_flag = ""
    esm_no_gpu_flag = ""
...
if args.hts:
    ...
    do(cmd)
    cmd = f"{cuda_env_prefix} {python} {script_folder}/esm/scripts/extract.py esm2_t33_650M_UR50D data/prepared_for_esm_{header}.fasta data/esm2_output --repr_layers 33 --include per_tok --truncation_seq_length 10000 --model_dir {script_folder}/esm_models{esm_no_gpu_flag}"
    do(cmd)
    cmd = f"{cuda_env_prefix} {python} {script_folder}/screening.py --seed {args.seed} --ckpt {ckpt} {protein_dynamic}"
    cmd += f" --save_visualisation --model_dir {model_workdir}  --protein_ligand_csv {ligandFile_with_protein_path} "
    cmd += f" --esm_embeddings_path data/esm2_output --out_dir {args.results}/{header} --inference_steps {args.inference_steps} --samples_per_complex {args.samples_per_complex} --savings_per_complex {args.savings_per_complex} --batch_size 5 --actual_steps {args.inference_steps} --no_final_step_noise{no_gpu_flag}"
    do(cmd)
    print("hts complete.")
else:
    if not args.no_inference:
        ...
        cmd = f"{cuda_env_prefix} {python} {script_folder}/esm/scripts/extract.py esm2_t33_650M_UR50D data/prepared_for_esm_{header}.fasta data/esm2_output --repr_layers 33 --include per_tok --truncation_seq_length 10000 --model_dir {script_folder}/esm_models{esm_no_gpu_flag}"
        do(cmd)
        cmd = f"{cuda_env_prefix} {python} {script_folder}/inference.py --seed {args.seed} --ckpt {ckpt} {protein_dynamic}"
        cmd += f" --save_visualisation --model_dir {model_workdir}  --protein_ligand_csv {ligandFile_with_protein_path} "
        cmd += f" --esm_embeddings_path data/esm2_output --out_dir {args.results}/{header} --inference_steps {args.inference_steps} --samples_per_complex {args.samples_per_complex} --savings_per_complex {args.savings_per_complex} --batch_size 5 --actual_steps {args.inference_steps} --no_final_step_noise{no_gpu_flag}"
        do(cmd)
    ...
    if not args.no_relax:
        cmd = f"{cuda_env_prefix} {relax_python} {script_folder}/relax_final.py --results_path {args.results}/{header} --samples_per_complex {args.samples_per_complex} --num_workers {args.num_workers}{no_gpu_flag}"
    ...
    if args.movie:
        for i in range(len(ligands)):
            cmd = f"{cuda_env_prefix} {relax_python} {script_folder}/movie_generation.py {args.results}/{header}/index{i}_idx_{i} 1 --python {python} --relax_python {relax_python} --inference_steps {args.inference_steps}"
            do(cmd)
            print(cmd)
